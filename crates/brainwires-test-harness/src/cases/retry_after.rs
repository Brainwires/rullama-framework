...
use brainwires_call_policy::{RetryPolicy, RetryProvider};
use brainwires_core::message::Usage;
use brainwires_core::{
...
impl EvaluationCase for RetryObeysRetryAfterHeader {
    ...
    async fn run(&self, trial_id: usize) -> Result<TrialResult> {
        let inner: Arc<dyn Provider> = Arc::new(OneErrThenOk {
            ...
            // string search; this format matches what brainwires-provider
...
inventory::submit! {
    TierACase {
        path: "brainwires_test_harness::cases::retry_after::RetryObeysRetryAfterHeader",
