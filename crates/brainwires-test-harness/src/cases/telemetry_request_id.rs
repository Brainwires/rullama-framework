...
impl EvaluationCase for CostByRequestIdIsolatesCalls {
    ...
    async fn run(&self, trial_id: usize) -> Result<TrialResult> {
        ...
        let collector =
            AnalyticsCollector::new(vec![Box::new(sink) as Box<dyn brainwires_telemetry::AnalyticsSink>]);
...
inventory::submit! {
    TierACase {
        path: "brainwires_test_harness::cases::telemetry_request_id::CostByRequestIdIsolatesCalls",
        crate_name: "brainwires-telemetry",
