...
//!
//! Convention: use `BRAINWIRES_LIVE_*` rather than reading raw `OPENAI_API_KEY`
//! / `ANTHROPIC_API_KEY` directly, so the harness can never accidentally pick
//! up keys from an unrelated CI environment.

/// Returns the Ollama base URL if `BRAINWIRES_LIVE_OLLAMA_BASE` is set.
pub fn live_ollama_base() -> Option<String> {
    std::env::var("BRAINWIRES_LIVE_OLLAMA_BASE").ok()
}

/// Returns the Ollama model name if `BRAINWIRES_LIVE_OLLAMA_MODEL` is set,
/// defaulting to `gemma4:e2b` when only the base URL is configured.
pub fn live_ollama_model() -> String {
    std::env::var("BRAINWIRES_LIVE_OLLAMA_MODEL").unwrap_or_else(|_| "gemma4:e2b".to_string())
}

/// Returns the OpenAI API key if `BRAINWIRES_LIVE_OPENAI_KEY` is set.
pub fn live_openai_key() -> Option<String> {
    std::env::var("BRAINWIRES_LIVE_OPENAI_KEY").ok()
}

/// Returns the OpenAI model name, defaulting to `gpt-5-nano`.
pub fn live_openai_model() -> String {
    std::env::var("BRAINWIRES_LIVE_OPENAI_MODEL").unwrap_or_else(|_| "gpt-5-nano".to_string())
}

/// Returns the Anthropic API key if `BRAINWIRES_LIVE_ANTHROPIC_KEY` is set.
pub fn live_anthropic_key() -> Option<String> {
    std::env::var("BRAINWIRES_LIVE_ANTHROPIC_KEY").ok()
}

/// Returns the Anthropic model name, defaulting to `claude-haiku-4-5`.
pub fn live_anthropic_model() -> String {
    std::env::var("BRAINWIRES_LIVE_ANTHROPIC_MODEL")
