...
async fn test_jsonrpc_cancel_task() {
    ...
    let result = rullama_a2a::server::jsonrpc_router::dispatch(&handler, &send_req)
        .await
        .unwrap()
        .unwrap();
    ...
    let result = rullama_a2a::server::jsonrpc_router::dispatch(&handler, &cancel_req)
        .await
        .unwrap()
        .unwrap();
...
async fn test_jsonrpc_list_tasks() {
    ...
    for msg in ["First", "Second"] {
        ...
        rullama_a2a::server::jsonrpc_router::dispatch(&handler, &req)
            .await
            .unwrap();
    }
    ...
    let result = rullama_a2a::server::jsonrpc_router::dispatch(&handler, &list_req)
        .await
        .unwrap()
        .unwrap();
...
async fn test_rest_subscribe_to_task() {
    ...
    .unwrap();
    match result {
        rullama_a2a::server::rest_router::RestResult::Stream(stream) => {
            let items: Vec<_> = stream.collect().await;
...
async fn test_sse_response_jsonrpc_format() {
    use rullama_a2a::server::sse_response;
...
async fn test_jsonrpc_push_config_set_unsupported() {
    ...
    match result {
        Err(resp) => {
            ...
            assert_eq!(err.code, rullama_a2a::error::PUSH_NOT_SUPPORTED);
        }
...
async fn test_rest_push_config_create_unsupported() {
    ...
    .await;
    match result {
        Err(e) => assert_eq!(e.code, rullama_a2a::error::PUSH_NOT_SUPPORTED),
...
fn test_error_constructors() {
    ...
    assert_eq!(e.code, rullama_a2a::error::TASK_NOT_FOUND);
    assert!(e.message.contains("t-1"));

    let e = A2aError::task_not_cancelable("t-2");
    assert_eq!(e.code, rullama_a2a::error::TASK_NOT_CANCELABLE);

    let e = A2aError::push_not_supported();
    assert_eq!(e.code, rullama_a2a::error::PUSH_NOT_SUPPORTED);

    let e = A2aError::unsupported_operation("grpc");
    assert_eq!(e.code, rullama_a2a::error::UNSUPPORTED_OPERATION);

    let e = A2aError::content_type_not_supported("video/mp4");
    assert_eq!(e.code, rullama_a2a::error::CONTENT_TYPE_NOT_SUPPORTED);

    let e = A2aError::invalid_request("too big");
    assert_eq!(e.code, rullama_a2a::error::INVALID_REQUEST);

    let e = A2aError::internal("oops");
    assert_eq!(e.code, rullama_a2a::error::INTERNAL_ERROR);

    let e = A2aError::method_not_found("foo/bar");
    assert_eq!(e.code, rullama_a2a::error::METHOD_NOT_FOUND);

    let e = A2aError::invalid_params("missing field");
    assert_eq!(e.code, rullama_a2a::error::INVALID_PARAMS);

    let e = A2aError::parse_error("bad json");
    assert_eq!(e.code, rullama_a2a::error::JSON_PARSE_ERROR);

    let e = A2aError::extended_card_not_configured();
    assert_eq!(e.code, rullama_a2a::error::EXTENDED_CARD_NOT_CONFIGURED);
}
