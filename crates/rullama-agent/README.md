...
[![Crates.io](https://img.shields.io/crates/v/rullama-agent.svg)](https://crates.io/crates/rullama-agent)
...
`rullama-agent` provides the multi-agent infrastructure for autonomous task execution. Agents run in a shared pool, communicate through a central hub, coordinate file and resource access via RAII lock guards, and pass through a validation gate before reporting success.
...
```rust
use std::sync::Arc;
use rullama_agent::prelude::*;
use rullama_core::Task;
...
| `seal-knowledge` | No | BKS/PKS knowledge system integration for SEAL via `rullama-knowledge` |
| `seal-feedback` | No | Audit feedback bridge for SEAL via `rullama-permission` |
...
```rust
use rullama_agent::{TaskAgentConfig, ValidationConfig, ValidationCheck};
...
```rust
use std::sync::Arc;
use rullama_agent::{FileLockManager, LockType};
...
```rust
use std::sync::Arc;
use rullama_agent::{CommunicationHub, AgentMessage};
...
```rust
use rullama_agent::SagaExecutor;
...
use rullama_agent::{GitCoordinator, git_tools, get_lock_requirements};
...
```rust
use rullama_agent::{extract_confidence, ResponseConfidence};
...
```rust
use rullama_agent::roles::AgentRole;
...
```rust
use rullama_agent::system_prompts::{AgentPromptKind, build_agent_prompt};
use rullama_agent::roles::AgentRole;
...
```rust
use rullama_agent::{ValidationConfig, ValidationCheck};
...
```rust
use rullama_agent::workflow::{WorkflowBuilder, WorkflowContext};
...
```rust
use rullama_reasoning::strategies::*;
...
```rust
use rullama_agent::otel::{export_to_otel, telemetry_attributes};
use opentelemetry::global;

let tracer = global::tracer("rullama-agent");
...
use rullama_core::graph::{EntityStore, RelationshipGraph};
