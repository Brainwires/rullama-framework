...
// ── LLM-driven workhorses moved to rullama-inference in Phase 11f ─────────
...
// system_prompts, task_agent — see crates/rullama-inference/

// ── Personas (pluggable system-prompt assembly) ──────────────────────────────

pub mod personas;

// agent_hooks moved to rullama-inference in Phase 11f (TaskAgent-coupled).

// runtime + context moved to rullama-inference in Phase 11f (the
...
// pool moved to rullama-inference in Phase 11f (TaskAgent pool, not generic).
pub mod roles;

// ── Core components ──────────────────────────────────────────────────────────

pub mod communication;
// `confidence` moved to `rullama-core` in Phase 11a; the agent-side
// shim was dropped in Phase 11g. Use `rullama_core::confidence::*`
// directly.
pub mod file_locks;
...
//
// Moved out of rullama-network in Phase 2 — both modules import only
// rullama_core/serde/anyhow/async_trait, so they belong here with the
...
// plan_executor moved to rullama-inference in Phase 11f.

// task_orchestrator moved to rullama-inference in Phase 11f
...
// validation_agent + validation_loop moved to rullama-inference in Phase 11f.
#[cfg(feature = "native")]
pub mod worktree;

// ── Re-exports ───────────────────────────────────────────────────────────────

// agent_hooks re-exports moved to rullama-inference in Phase 11f
// (the trait references TaskAgentResult, which is TaskAgent-specific).

// AgentRuntime + run_agent_loop re-exports moved to rullama-inference in
// Phase 11f.
...
// confidence types moved to rullama-core in Phase 11a; re-export shim
// dropped in Phase 11g. Import from `rullama_core::confidence::*` directly.
...
// Task orchestration re-exports moved to rullama-inference in Phase 11f
// (TaskAgent-coupled).
...
// validation_agent, validation_loop, cycle_orchestrator, plan_executor,
// summarization, system_prompts, task_agent) — extracted to rullama-inference
// in Phase 11f. Import from `rullama_inference::*` directly or via
// `rullama::inference::*` (facade).
...
/// summarization) live in `rullama-inference` since Phase 11f. Import
/// from there directly or use the umbrella `rullama::inference` module.
pub mod prelude {
    // Schema + lifecycle (AgentContext lives in rullama-inference now)
    ...
    pub use rullama_core::confidence::{ConfidenceFactors, ResponseConfidence};
