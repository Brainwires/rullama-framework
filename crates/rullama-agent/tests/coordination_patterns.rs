...
use rullama_agent::contract_net::{
    BidEvaluationStrategy, ContractNetManager, ContractParticipant, TaskAnnouncement, TaskBid,
    TaskRequirements, TaskStatus,
};
use rullama_agent::optimistic::{CommitResult, OptimisticController, ResolutionStrategy};
use rullama_agent::saga::{NoOpCompensation, SagaExecutor, SagaOperationType, SagaStatus};
use std::sync::Arc;
...
async fn saga_with_optimistic_version_check() {
    ...
    let stale = rullama_agent::optimistic::OptimisticToken {
