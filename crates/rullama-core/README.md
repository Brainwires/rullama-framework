...
[![Crates.io](https://img.shields.io/crates/v/rullama-core.svg)](https://crates.io/crates/rullama-core)
...
`rullama-core` defines the foundational types shared by every other crate in the framework: messages, tools, providers, tasks, plans, permissions, embeddings, vector stores, and knowledge graphs. It is intentionally lightweight, with no runtime dependencies beyond serialization and async primitives.
...
```rust
use rullama_core::{Message, Tool, ToolInputSchema, ChatOptions, Provider};
...
```rust
use rullama_core::workflow_state::{
    FsWorkflowStateStore, SideEffectRecord, WorkflowStateStore,
};

let store = FsWorkflowStateStore::with_default_path()?;  // ~/.rullama/workflow/
...
```rust
use rullama_core::event::{EventEnvelope, new_trace_id};
...
```rust
use rullama_core::{Message, ContentBlock, MessageContent};
...
```rust
use rullama_core::{Tool, ToolInputSchema, ToolResult};
use std::collections::HashMap;
...
```rust
use rullama_core::{WorkingSet, WorkingSetConfig};
...
```rust
use rullama_core::{PlanBudget, SerializablePlan, PlanStep};
...
```rust
use rullama_core::{Task, TaskPriority};
