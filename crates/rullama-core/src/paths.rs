...
/// Default: "rullama-rag"
/// With `alt-folder-name` feature: Uses alternative folder name "rullama" instead of "rullama-rag".
#[cfg(not(feature = "alt-folder-name"))]
const PROJECT_FOLDER_NAME: &str = "rullama-rag";

#[cfg(feature = "alt-folder-name")]
const PROJECT_FOLDER_NAME: &str = "rullama";
...
impl PlatformPaths {
    ...
    ///
    /// Returns: ~/.rullama/cache/fastembed
    ///
    /// Models are global (not per-project), so this is a single shared location
    /// consistent with the rest of the framework's use of ~/.rullama/.
    ///
    ...
    pub fn default_fastembed_cache_path() -> PathBuf {
        PathBuf::from(".rullama").join("cache").join("fastembed")
    }
...
mod tests {
    ...
    fn test_project_paths_contain_project_name() {
        ...
        assert!(data_dir.to_string_lossy().contains("rullama-rag"));
        assert!(cache_dir.to_string_lossy().contains("rullama-rag"));
        assert!(config_dir.to_string_lossy().contains("rullama-rag"));
    }

    #[test]
    fn test_default_lancedb_path() {
        let path = PlatformPaths::default_lancedb_path();
        assert!(path.to_string_lossy().contains("rullama-rag"));
    ...
    fn test_specific_file_paths() {
        ...
        for path in [
            ...
            assert!(
                path.to_string_lossy().contains("rullama-rag"),
                "Path {:?} should contain 'rullama-rag'",
                path
            );
        }
