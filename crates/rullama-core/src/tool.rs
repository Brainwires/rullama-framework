...
///
/// Defined in `rullama-core` so that [`ToolContext`] can hold an
/// `Arc<dyn StagingBackend>` without depending on `rullama-tools`,
/// which would create a circular crate dependency.
///
/// The concrete implementation lives in `rullama-tools::transaction::TransactionManager`.
...
///
/// Populated by `write_file` (in `rullama-tools`) after its post-write
/// read-back succeeds, and read by the validation loop (in `rullama-agent`)
