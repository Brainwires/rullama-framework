...
/// Stores workflow checkpoints as JSON files under `~/.rullama/workflow/`.
///
...
impl FsWorkflowStateStore {
    /// Create a store using the default directory (`~/.rullama/workflow/`).
    pub fn default_path() -> Result<PathBuf> {
        let home = dirs::home_dir().context("cannot determine home directory")?;
        Ok(home.join(".rullama").join("workflow"))
    }
