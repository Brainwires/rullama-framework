...
//! context management.
//!
//! Unlike the observational [`LifecycleEvent`][rullama_core::lifecycle::LifecycleEvent]
//! system in `rullama-core`, these hooks can **control** the loop — skip
...
//! use rullama_agent::AgentContext;
//!
//! struct MyHooks;
//!
...
use rullama_core::{Message, Role, ToolResult, ToolUse, estimate_tokens_from_size};
        ...
        self.messages
            ...
            .map(|m| {
                let bytes = match &m.content {
                    rullama_core::MessageContent::Text(t) => t.len() as u64,
                    rullama_core::MessageContent::Blocks(blocks) => {
...
/// manipulation.
///
/// # Relationship to `rullama_core::lifecycle`
///
...
pub trait AgentLifecycleHooks: Send + Sync {
    ...
    async fn on_after_provider_call(
        &self,
        _ctx: &IterationContext<'_>,
        _response: &rullama_core::ChatResponse,
    ) {
    }
...
impl DefaultDelegationHandler {
    ...
    pub async fn delegate(&self, request: &DelegationRequest) -> Result<DelegationResult> {
        let task = rullama_core::Task::new(
...
mod tests {
    use super::*;
    use rullama_core::{Message, MessageContent, Role};
