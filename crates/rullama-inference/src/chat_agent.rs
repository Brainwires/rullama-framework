...
use rullama_tool_runtime::{PreHookDecision, ToolExecutor, ToolPreHook};
use uuid::Uuid;
...
/// 4-chars-per-token heuristic in `rullama_call_policy::budget` —
/// imprecise but cheap and dep-free. When `rullama-call-policy`'s
...
impl ChatAgent {
    ...
    /// `rullama_call_policy::ResilienceError::BudgetExceeded` error (wrapped
    /// in `anyhow::Error`).
    ///
    ...
    /// [`TurnReport`](rullama_core::TurnReport) of token / duration usage for this single turn.
    ///
    ...
    pub async fn process_message_with_report(
        &mut self,
        input: &str,
    ) -> Result<(String, rullama_core::TurnReport)> {
...
async fn execute_one_tool(
    ...
    let result = match executor.execute(tu, &exec_ctx).await {
        Ok(r) => r,
        Err(e) => {
            rullama_core::ToolResult::error(tu.id.clone(), format!("tool executor error: {e}"))
        }
    };
    ContentBlock::ToolResult {
        tool_use_id: tu.id.clone(),
...
mod tests {
    use super::*;
    use rullama_core::{ToolContext, ToolInputSchema};
