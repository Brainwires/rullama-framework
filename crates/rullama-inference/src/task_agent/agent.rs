...
use rullama_tool_runtime::{PreHookDecision, wrap_with_content_source};
...
use rullama_agent::communication::AgentMessage;
use rullama_agent::execution_graph::{ExecutionGraph, RunTelemetry, ToolCallRecord};
use rullama_agent::file_locks::LockType;
...
impl TaskAgent {
    ...
    async fn attempt_validated_completion(
        ...
        pre_execution_plan: Option<rullama_core::SerializablePlan>,
    ...
    async fn execute_impl(&self) -> Result<TaskAgentResult> {
        ...
        let mut pre_execution_plan: Option<rullama_core::SerializablePlan> = None;
        if let Some(ref budget) = self.config.plan_budget {
            ...
            let planning_options = rullama_core::ChatOptions {
                ...
                .await
            {
                Ok(response) => {
                    ...
                    if let Some(plan) = rullama_core::SerializablePlan::parse_from_text(
        ...
        let tool_context = ToolContext {
            ...
            idempotency_registry: Some(rullama_core::IdempotencyRegistry::new()),
        ...
        loop {
            ...
            if let Some(ref hook) = self.config.billing_hook {
                let event = rullama_telemetry::UsageEvent::tokens(
                    self.id.clone(),
            ...
            for tool_use in &tool_uses {
                ...
                #[cfg(feature = "telemetry")]
                if let Some(ref hook) = self.config.billing_hook {
                    let pending = rullama_telemetry::UsageEvent::tool_call(
                        self.id.clone(),
                        tool_use.name.clone(),
                    );
                ...
                #[cfg(feature = "telemetry")]
                if let Some(ref hook) = self.config.billing_hook {
                    let event = rullama_telemetry::UsageEvent::tool_call(
                        self.id.clone(),
                        tool_use.name.clone(),
                    );
                    if let Err(e) = hook.0.on_usage(&event).await {
