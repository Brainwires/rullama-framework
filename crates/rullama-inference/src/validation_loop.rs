...
//!
//! When the `tools` feature is enabled, uses rullama-tools validation functions
//! (check_duplicates, verify_build, check_syntax). Without it, those checks are skipped.

use anyhow::Result;
use rullama_core::IntendedWrites;
...
async fn run_duplicates_check(changed_files: &[String], issues: &mut Vec<ValidationIssue>) {
    use rullama_tool_runtime::validation::check_duplicates;
...
async fn run_syntax_check(changed_files: &[String], issues: &mut Vec<ValidationIssue>) {
    use rullama_tool_runtime::validation::check_syntax;
...
async fn run_build_check(
    ...
    use rullama_tool_runtime::validation::verify_build;
