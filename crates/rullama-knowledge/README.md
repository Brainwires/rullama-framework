...
`rullama-knowledge` consolidates three previously separate crates (`rullama-brain`, `rullama-prompting`, `rullama-rag`) into a single coherent intelligence layer. It provides persistent thought storage with semantic search, adaptive prompting technique selection, codebase indexing with hybrid retrieval, spectral diversity reranking, and AST-aware code analysis.
...
```toml
[dependencies]
rullama-knowledge = "0.11"
```

Capture a thought and search memory:

```rust
use rullama_knowledge::knowledge::{BrainClient, CaptureThoughtRequest, SearchMemoryRequest};
...
# Full native build
rullama-knowledge = { version = "0.11", features = ["native"] }

# RAG only
rullama-knowledge = { version = "0.11", default-features = false, features = ["rag"] }

# WASM target
rullama-knowledge = { version = "0.11", default-features = false, features = ["wasm"] }
```
...
Persistent thought storage, entity graphs, and knowledge systems. Formerly the standalone `rullama-brain` crate.
...
| Method | Description |
|--------|-------------|
| `new()` | Create client with default paths (`~/.rullama/`) |
...
**Known limitation**: `ln(1) = 0`, so the mention-count component contributes nothing for entities seen exactly once. The type bonus and recency proxy still apply, so the score is always non-zero — but a single-mention entity's score depends solely on its type and message spread. Empirical validation via `rullama_autonomy::eval::EntitySingleMentionCase` confirms non-zero scoring is maintained.
...
A standalone MCP server binary for the knowledge subsystem is available at `extras/rullama-brain-server/`.

## Prompting Subsystem

*Feature: `prompting` (default)*

Adaptive prompting technique selection based on arXiv:2510.18162, with BKS/PKS/SEAL integration. Formerly the standalone `rullama-prompting` crate.
...
```rust
use rullama_knowledge::prompting::{PromptGenerator, GeneratedPrompt};
...
Codebase indexing and semantic search with hybrid retrieval. Formerly the standalone `rullama-rag` crate.
...
```rust
use rullama_knowledge::rag::{RagClient, SearchGitHistoryRequest};
...
```rust
use rullama_knowledge::rag::Config;

let config = Config {
    vector_db: VectorDbConfig {
        backend: "lancedb".into(),
        lancedb_path: "~/.rullama/rag".into(),
        ..Default::default()
    },
...
A standalone MCP server binary for the RAG subsystem is available at `extras/rullama-rag-server/`.
...
```rust
use rullama_knowledge::spectral::{SpectralReranker, SpectralSelectConfig};
...
```toml
[dependencies]
rullama = { version = "0.11", features = ["cognition"] }
```

Or depend on `rullama-knowledge` directly:
...
| Old path | New path |
|----------|----------|
| `rullama_brain::BrainClient` | `rullama_knowledge::knowledge::BrainClient` |
| `rullama_brain::EntityStore` | `rullama_knowledge::knowledge::EntityStore` |
| `rullama_prompting::TechniqueLibrary` | `rullama_knowledge::prompting::TechniqueLibrary` |
| `rullama_prompting::PromptGenerator` | `rullama_knowledge::prompting::PromptGenerator` |
| `rullama_rag::RagClient` | `rullama_knowledge::rag::RagClient` |
| `rullama_rag::Config` | `rullama_knowledge::rag::Config` |

Most types are also re-exported at the crate root and in the `prelude` module:

```rust
use rullama_knowledge::prelude::*;
