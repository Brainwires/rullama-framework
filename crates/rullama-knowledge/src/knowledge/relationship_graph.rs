...
pub use rullama_core::graph::{EdgeType, GraphEdge, GraphNode};
...
impl RelationshipGraph {
    ...
    // Spectral graph methods (spectral_clusters / spectral_central_nodes /
    // connectivity / sparsify) used to live here behind a `spectral` feature
    // but their real implementation lives in `rullama-rag::spectral`. The
    // versions here referenced `crate::spectral::graph_ops::*` which never
    // existed in this crate; enabling the feature broke compilation. Removed
    // (see `rullama_rag::spectral::graph_ops` for the live API). Callers
    // who need spectral analysis on a `RelationshipGraph` should call into
    // the rag crate's API directly.
}

impl rullama_core::graph::RelationshipGraphT for RelationshipGraph {
