...
- **Offline config persistence** — `McpConfigManager` saves server configurations to `~/.rullama/mcp-config.json`
...
```text
  ...
  │                                                                   │
  │  ┌─── McpConfigManager ───────────────────────────────────────┐  │
  │  │  load() / save() ◄──► ~/.rullama/mcp-config.json        │  │
...
| `wasm` | No | WASM-compatible build (no transport, no config persistence, forwards `rullama-core/wasm`) |
...
| `load()` | Load from `~/.rullama/mcp-config.json` (creates default if missing) |
...
Config file location: `~/.rullama/mcp-config.json`
