...
[![Crates.io](https://img.shields.io/crates/v/rullama-mcp-server.svg)](https://crates.io/crates/rullama-mcp-server)
...
`rullama-mcp-server` provides everything needed to build an MCP-compliant tool server:
...
This crate was extracted from `rullama-network` so that consumers who only need to build MCP servers don't have to pull in the full networking stack.
...
use rullama_mcp::types::{ServerInfo, ServerCapabilities, CallToolResult};
...
```rust
use rullama_mcp_server::{McpToolRegistry, McpToolDef, ToolHandler};
use rullama_mcp::types::CallToolResult;
use serde_json::Value;
