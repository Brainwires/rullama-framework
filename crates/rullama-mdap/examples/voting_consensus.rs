...
//!
//! Run with: `cargo run -p rullama-agent --features mdap --example voting_consensus`

use rullama_mdap::red_flags::{
    OutputFormat, RedFlagConfig, RedFlagValidator, StandardRedFlagValidator,
};
use rullama_mdap::voting::{ResponseMetadata, SampledResponse, VoteResult, VotingMethod};
    ...
    for (i, resp) in responses.iter().enumerate() {
        ...
        let status = if result.is_valid() {
            ...
            format!(
                "FLAGGED: {}",
                match &result {
                    rullama_mdap::red_flags::RedFlagResult::Flagged { reason, severity } => {
