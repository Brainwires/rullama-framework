...
[![Crates.io](https://img.shields.io/crates/v/rullama-network.svg)](https://crates.io/crates/rullama-network)
...
`rullama-network` provides the full networking stack for AI agents: a 5-layer protocol stack for pluggable agent communication, encrypted IPC, a remote bridge for backend connectivity, agent lifecycle management, device allowlists, permission relay, and optional distributed mesh networking.

> **Note:** The MCP server framework (McpServer, McpHandler, McpToolRegistry, middleware pipeline) has been extracted into [`rullama-mcp-server`](../rullama-mcp-server/README.md). Use that crate if you only need to build MCP tool servers without the full networking stack.
...
```rust
use rullama_agent_network::{
    NetworkManagerBuilder, AgentIdentity, Payload, NetworkEvent,
};
use rullama_agent_network::transport::TcpTransport;
use rullama_agent_network::routing::DirectRouter;
use rullama_agent_network::discovery::ManualDiscovery;
...
| `a2a-transport` | No | A2A protocol transport (requires `rullama-a2a`) |
...
```rust
use rullama_agent_network::identity::{AgentIdentity, AgentCard};
...
The MCP server framework has been extracted into [`rullama-mcp-server`](../rullama-mcp-server/README.md). See that crate for McpServer, McpHandler, McpToolRegistry, and the middleware pipeline. `rullama-network` re-exports the mcp-server crate for consumers who need both.
...
**Encryption (`IpcCipher`):** Derives a 256-bit key from the session token via SHA-256 with domain separator `rullama-ipc-v1:`. All post-handshake messages use ChaCha20-Poly1305 authenticated encryption. Wire format: `[nonce 12B][ciphertext + auth tag 16B]`.
...
For MCP server functionality, use `rullama-mcp-server` directly or via the re-export:

```rust
use rullama_mcp_server::{
...
```rust
use rullama_agent_network::transport::{TcpTransport, TransportAddress, Transport};
use rullama_agent_network::network::{MessageEnvelope, Payload};
...
```rust
use rullama_agent_network::transport::{PubSubTransport, TransportAddress, Transport};
use rullama_agent_network::network::{MessageEnvelope, Payload};
...
```rust
use rullama_agent_network::transport::{A2aTransport, TransportAddress, Transport};
...
```rust
use rullama_agent_network::ipc::{IpcConnection, ViewerMessage, AgentMessage};
