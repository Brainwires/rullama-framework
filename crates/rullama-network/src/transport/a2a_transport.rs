...
pub struct A2aTransport {
    ...
    client: Arc<rullama_a2a::A2aClient>,
    /// Whether connected.
    connected: bool,
    /// Remote endpoint URL.
...
impl A2aTransport {
    ...
    pub fn new(client: rullama_a2a::A2aClient) -> Self {
    ...
    pub fn from_url(url: impl Into<String>) -> Result<Self> {
        ...
        let client = rullama_a2a::A2aClient::new_jsonrpc(parsed);
        let (incoming_tx, incoming_rx) = broadcast::channel(256);
        Ok(Self {
            client: Arc::new(client),
            connected: false,
    ...
    pub fn a2a_client(&self) -> &rullama_a2a::A2aClient {
        &self.client
    }
}
...
impl Transport for A2aTransport {
    ...
    async fn send(&self, envelope: &MessageEnvelope) -> Result<()> {
        ...
        let mut message = rullama_a2a::Message::user_text(&text);
        ...
        let req = rullama_a2a::SendMessageRequest {
            message,
            tenant: None,
            configuration: None,
            metadata: None,
...
/// Convert an incoming A2A [`Message`](rullama_a2a::Message) to a
/// [`MessageEnvelope`].
///
...
mod tests {
    ...
    fn a2a_transport_type() {
        ...
        let client = rullama_a2a::A2aClient::new_jsonrpc(url);
    ...
    fn a2a_message_conversion() {
        let msg = rullama_a2a::Message::user_text("Hello from A2A");
