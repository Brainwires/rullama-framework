...
[![Crates.io](https://img.shields.io/crates/v/rullama-permission.svg)](https://crates.io/crates/rullama-permission)
...
```rust
use rullama_permissions::CapabilityProfile;
...
```rust
use rullama_permissions::PathPattern;
...
Persisted to `~/.rullama/trust_store.json`.
...
TOML-based configuration loaded from `~/.rullama/permissions.toml`.
...
|----------|-------------|
| `default_permissions_path()` | Returns `~/.rullama/permissions.toml` |
| `ensure_permissions_dir()` | Creates `~/.rullama/` if it doesn't exist |
...
```rust
use rullama_permissions::{AgentCapabilities, CapabilityProfile};
...
```rust
use rullama_permissions::{TrustManager, TrustLevel, ViolationSeverity};
...
```rust
use rullama_permissions::{AnomalyDetector, AnomalyConfig};
...
```rust
use rullama_permissions::{PermissionsConfig, default_permissions_path};

// Load from default path (~/.rullama/permissions.toml)
...
| File | Description |
|------|-------------|
| `~/.rullama/permissions.toml` | Permission configuration |
