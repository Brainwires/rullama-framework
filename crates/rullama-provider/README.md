...
[![Crates.io](https://img.shields.io/crates/v/rullama-provider.svg)](https://crates.io/crates/rullama-provider)
...
`rullama-provider` provides concrete implementations of the `Provider` trait for multiple AI services: Anthropic (Claude), OpenAI (GPT), Google (Gemini), Ollama, and local LLM inference via llama.cpp. Every provider converts to and from the unified `rullama-core` message types, so callers can swap backends without changing application code.

**Design principles:**

- **Unified interface** — all providers implement the same `Provider` trait from `rullama-core` (chat, streaming, tool calling)
...
- **Tool calling** — Anthropic, OpenAI, Google, and Ollama all support function calling mapped to/from `rullama_core::Tool`
...
```rust
use rullama_providers::{AnthropicProvider, Provider, ChatOptions};
...
All providers implement the `Provider` trait from `rullama-core`. This is the unified interface that callers program against.
...
`FromStr` also accepts aliases: `"gemini"` maps to `Google`, `"rullama"` maps to `Custom`.
...
Manages registered local models with persistence to `~/.config/rullama/local_models.json`.

| Method | Description |
|--------|-------------|
| `new()` | Create an empty registry |
| `with_default_dir()` | Create with default models directory (`~/.local/share/rullama/models/`) |
...
```rust
use rullama_providers::{OpenAIProvider, Provider, ChatOptions};
use rullama_core::message::{Message, StreamChunk};
...
```rust
use rullama_providers::{RateLimitedClient, RateLimiter};
...
```rust
use rullama_providers::{ProviderType, ProviderConfig};
...
```rust
use rullama_providers::{LocalLlmProvider, LocalLlmConfig, LocalInferenceParams};
...
use rullama_providers::{Provider, ChatOptions};
use rullama_core::message::Message;
...
```rust
use rullama_providers::{LocalModelRegistry, LocalLlmConfig, known_models, get_known_model};
use std::path::PathBuf;
...
```rust
use rullama_providers::{LocalLlmPool, LocalLlmConfig};
use std::path::PathBuf;
...
Use via the `rullama` facade crate with the `providers` feature, or depend on `rullama-provider` directly:
...
```rust
use rullama_providers::{
    // Trait + options (from rullama-core)
