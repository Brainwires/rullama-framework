...
/// `rullama_core::Provider` trait.
///
/// All `rullama_core` message / tool types are converted to and from the
...
pub struct OpenAiChatProvider {
    ...
    analytics_collector: Option<std::sync::Arc<rullama_telemetry::AnalyticsCollector>>,
}
...
/// Convert rullama-core `Message` values into OpenAI wire messages.
...
/// Convert rullama-core `Tool` values into OpenAI wire tools.
...
/// Parse a raw `OpenAIResponse` into the rullama-core `ChatResponse`.
