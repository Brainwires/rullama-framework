...
```rust
use std::sync::Arc;
use rullama_reasoning::{
    LocalInferenceConfig, LocalRouterBuilder, JsonOutputParser, OutputParser,
    parse_plan_steps,
};

# async fn demo(provider: Arc<dyn rullama_core::Provider>) -> anyhow::Result<()> {
...
```rust
use rullama_reasoning::LocalInferenceConfig;
...
- **`RelevanceScorer`** — rerank retrieved chunks by query-aware
  relevance; pairs well with `rullama-knowledge::SpectralReranker` for
...
moving the `prompting` subsystem here. It stayed in `rullama-knowledge`
...
should continue to target `rullama_knowledge::prompting`.
