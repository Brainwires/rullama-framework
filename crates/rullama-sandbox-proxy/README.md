...
Egress allowlist HTTP proxy used by `rullama-sandbox` to enforce
`NetworkPolicy::Limited`. This crate ships a single binary —
`rullama-sandbox-proxy` — that listens for plain HTTP requests (absolute
...
```
docker build -t ghcr.io/brainwires/rullama-sandbox-proxy:latest \
...
`rullama-sandbox`'s `DockerSandbox` creates an ephemeral bridge network
...
`crates/rullama-sandbox/src/docker.rs`.
