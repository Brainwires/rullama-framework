...
# async fn demo() -> rullama_sandbox::Result<()> {
...
- **`Limited(hosts)`** — sandbox lives on an `internal: true` docker
  network with no default route. A `rullama-sandbox-proxy` sidecar is
...
`rullama-tool-builtins` uses the `Sandbox` trait to execute bash / python tool
calls under isolation; `rullama-agent` composes per-agent sandbox
policies.
