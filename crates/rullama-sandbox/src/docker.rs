...
//!   A sidecar `rullama-sandbox-proxy` container is attached to BOTH that
...
impl DockerSandbox {
    ...
    async fn setup_limited_network(
        ...
        let net_name = format!("rullama-sandbox-net-{}", handle.as_uuid());
...
impl Sandbox for DockerSandbox {
    async fn spawn(&self, spec: ExecSpec) -> Result<ExecHandle> {
        ...
        let name = format!("rullama-sandbox-{}", handle.as_uuid());
