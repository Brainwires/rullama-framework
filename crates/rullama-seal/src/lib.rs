...
//! agent capabilities in rullama-cli.
//!
//! ## Components
//!
...
//!
//! ```rust,ignore
//! use rullama_seal::{SealProcessor, SealConfig};
//!
...
/// `extras/rullama-cli/src/storage/pattern_store.rs` because the
...
impl SealProcessor {
    ...
    #[cfg(feature = "mdap")]
    pub fn record_mdap_metrics(&mut self, metrics: &rullama_mdap::MdapMetrics) {
        if !self.config.enable_learning {
            return;
        }
