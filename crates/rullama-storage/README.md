...
[![Crates.io](https://img.shields.io/crates/v/rullama-storage.svg)](https://crates.io/crates/rullama-storage)
[![Documentation](https://img.shields.io/docsrs/rullama-storage)](https://docs.rs/rullama-storage)
[![License](https://img.shields.io/crates/l/rullama-storage.svg)](LICENSE)
...
>
> - **`rullama-stores`** — schema + CRUD for the opinionated minimum
...
> - **`rullama-memory`** — tiered hot/warm/cold agent memory
...
`rullama-storage` is the persistent-backend foundation: a generic
...
```text
  ...
  |  +------------------------------------------------------------------+  |
  |  Note: EntityStore and RelationshipGraph moved to rullama-knowledge |
  +------------------------------------------------------------------------+
```
...
```rust
use rullama_storage::{LanceDatabase, CachedEmbeddingProvider};
use rullama_stores::{MessageStore, MessageMetadata};
use std::sync::Arc;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    // Initialize storage — one struct, one connection
    let db = Arc::new(LanceDatabase::new("~/.rullama/db").await?);
...
```rust
use rullama_storage::{LanceDatabase, StorageBackend};
...
Text embedding with LRU caching, backed by FastEmbed (all-MiniLM-L6-v2, 384 dimensions). Implements the `rullama_core::EmbeddingProvider` trait.
...
| Method | Description |
|--------|-------------|
| `new_default()` | Use `~/.rullama/locks.db` |
...
```rust
use rullama_storage::{LanceDatabase, CachedEmbeddingProvider, MessageStore, MessageMetadata};
use std::sync::Arc;
...
let store = DocumentStore::new(db.clone(), embeddings.clone(), "~/.rullama/bm25");
...
Use via the `rullama` facade crate with the `storage` feature, or depend on `rullama-storage` directly:
