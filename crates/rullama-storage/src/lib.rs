...
//!
//! - **`rullama-memory`** — `MessageStore`, `SummaryStore`, `FactStore`,
//!   `MentalModelStore`, `TierMetadataStore`, `TieredMemory`.
//! - **`rullama-cli` `crate::storage`** — `ConversationStore`,
...
//!   the `ImageStore` that lives in `rullama-cli::storage`.
//!
//! # Feature Flags
//!
...
/// Image-storage type definitions (concrete `ImageStore` lives in `rullama-cli`).
pub mod image_types;
...
/// vector + keyword search; consumed by `rullama-rag` and other
/// indexers.
...
// Phase 9 moves:
//   paths + file_context → rullama-core (cross-cutting utilities, no storage internals)
...
// FileChunk / FileContent / FileContextManager moved to rullama-core in Phase 9.
...
pub mod prelude {
    ...
    // file_context moved to rullama-core in Phase 9.
