...
//! layer in the `rullama-memory` crate. They live here (in the schema
//! crate) so the schema stores can be used standalone without pulling
//! `rullama-memory` and so `rullama-memory` doesn't have to re-export
...
/// entries into the memory store. The `rullama-memory` crate gates
