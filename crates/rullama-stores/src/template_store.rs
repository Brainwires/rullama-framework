...
impl TemplateStore {
    ...
    ///
    /// Falls back to ~/.rullama/ if no platform data dir is available.
    pub fn with_default_dir() -> Result<Self> {
        let data_dir = dirs::data_dir()
            .unwrap_or_else(|| dirs::home_dir().unwrap_or_default().join(".rullama"));
        let data_dir = data_dir.join("rullama");
        Self::new(data_dir)
    }
}
