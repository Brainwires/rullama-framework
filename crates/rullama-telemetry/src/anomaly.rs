...
//! `rullama-permission::audit::AuditEvent`, but other crates can plug in
...
///
/// Concrete event-type enums (e.g., `AuditEventType` in `rullama-permission`)
...
/// `rullama-permission`); telemetry itself does not depend on permissions.
