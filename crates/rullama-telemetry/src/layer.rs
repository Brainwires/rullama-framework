...
/// |-----------------|------------------------|--------------------------------------|
/// | `provider.chat` | `provider`, `model`    | [`AnalyticsEvent::ProviderCall`] (partial — tokens/cost come from explicit emission in rullama-provider) |
///
    ...
    S: Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_close(&self, id: tracing::span::Id, ctx: Context<'_, S>) {
        ...
        if meta.name() == "provider.chat" {
            ...
            // explicitly by rullama-provider after each chat() call (Phase 2).
