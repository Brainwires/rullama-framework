...
//!
//! ```rust,no_run
//! use rullama_telemetry::{AnalyticsCollector, metrics::MetricsRegistry};
//!
...
impl OutcomeMetrics {
    ...
    ///
    /// (`CacheStrategy` is defined in `rullama_core::provider`.)
...
impl MetricsRegistry {
    ...
    ///
    /// ```text
    /// # HELP rullama_agent_runs_total Total agent runs attempted
    /// # TYPE rullama_agent_runs_total counter
    /// rullama_agent_runs_total{agent_id="code-review"} 42
    /// ```
    pub fn prometheus_text(&self) -> String {
        ...
        // ── rullama_agent_runs_success ────────────────────────────────────
        counter(
            &mut out,
            "rullama_agent_runs_success_total",
        ...
        // ── rullama_agent_runs_failure ────────────────────────────────────
        counter(
            &mut out,
            "rullama_agent_runs_failure_total",
...
mod tests {
    ...
    async fn prometheus_text_well_formed() {
        ...
        assert!(text.contains("# HELP rullama_agent_runs_total"));
        assert!(text.contains("# TYPE rullama_agent_runs_total counter"));
        assert!(text.contains("rullama_agent_runs_total{agent_id=\"code-review\"} 1"));
        assert!(text.contains("rullama_agent_success_rate{agent_id=\"code-review\"} 1.000000"));
        assert!(text.contains("rullama_agent_cost_usd_total{agent_id=\"code-review\"}"));
    }
    ...
    async fn cache_metrics_accumulate_and_compute_hit_rate() {
        ...
        let text = reg.prometheus_text();
        assert!(text.contains("rullama_agent_cache_read_tokens_total{agent_id=\"s1\"} 185"));
        assert!(text.contains("rullama_agent_cache_creation_tokens_total{agent_id=\"s1\"} 200"));
        assert!(text.contains("rullama_agent_cache_hit_rate{agent_id=\"s1\"}"));
    }
