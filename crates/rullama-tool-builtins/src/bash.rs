...
use rullama_core::{Tool, ToolContext, ToolInputSchema, ToolResult};
...
/// in by setting `RULLAMA_BASH_SANDBOX=network-deny` (or via the CLI
...
impl BashSandboxMode {
    ...
    pub fn from_env() -> Self {
        match std::env::var("RULLAMA_BASH_SANDBOX").as_deref() {
...
pub(crate) fn apply_sandbox(command: &str, mode: BashSandboxMode) -> SandboxedCommand {
    match mode {
        ...
        BashSandboxMode::NetworkDeny => {
            ...
            {
                tracing::warn!(
                    target: "rullama_tool_builtins::bash::sandbox",
...
mod tests {
    ...
    /// End-to-end: with RULLAMA_BASH_SANDBOX=network-deny, `echo` still
    ...
    /// parallel with other tests that read RULLAMA_BASH_SANDBOX. We
    ...
    fn test_network_deny_blocks_tcp_but_allows_echo() {
        ...
        unsafe {
            match prev {
                Some(v) => std::env::set_var("RULLAMA_BASH_SANDBOX", v),
                None => std::env::remove_var("RULLAMA_BASH_SANDBOX"),
            }
        }
    }
