...
//!   management (absorbed from `rullama-system`).
//!
...
//! crate — depend on `rullama-tool-runtime` directly, or use the
//! `rullama-tools` façade which re-exports both.
...
/// (absorbed from rullama-system).
#[cfg(feature = "system")]
pub mod system;

/// Sandboxed multi-language code interpreters (absorbed from rullama-code-interpreters).
#[cfg(feature = "interpreters")]
...
/// Build a [`rullama_tool_runtime::ToolRegistry`] pre-populated with
...
/// the `rullama-tools` façade composes those on top via its own
...
pub fn registry_with_builtins() -> rullama_tool_runtime::ToolRegistry {
    ...
    #[allow(unused_mut)]
    let mut registry = rullama_tool_runtime::ToolRegistry::with_runtime_meta_tools();
    ...
    #[cfg(feature = "native")]
    {
        ...
        registry.register_tools(rullama_tool_runtime::get_validation_tools());
    }
