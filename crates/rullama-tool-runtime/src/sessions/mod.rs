...
//! this module only defines the tool schemas plus a [`SessionBroker`](rullama_stores::session::broker::SessionBroker) trait
//! that the host implements over its actual registry.

// SessionBroker / SessionId / SessionMessage / SessionSummary / SpawnRequest /
// SpawnedSession live in `rullama-session::broker`. Depend on that crate
