...
//!
//! Run: cargo run -p rullama --example storage_with_rag --features memory,rag

use anyhow::Result;
use std::sync::Arc;
use std::time::{SystemTime, UNIX_EPOCH};

// Storage primitives come from `rullama::storage::*`; the per-domain
// `MessageStore` lives under `rullama::memory::*` (extracted in the
// rullama-memory split).
use rullama::memory::{MessageMetadata, MessageStore};
use rullama::storage::{CachedEmbeddingProvider, LanceDatabase};

// RAG types come from `rullama::rag::*`
use rullama::rag::{IndexRequest, QueryRequest, RagClient};
...
async fn main() -> Result<()> {
    ...
    let db_path = "/tmp/rullama_storage_rag_example";
    ...
    let index_req = IndexRequest {
        ...
        project: Some("rullama-facade".into()),
