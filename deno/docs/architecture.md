...
- **LanceDB / ONNX / tantivy RAG** — native indexing and embedding stays in
  Rust. The Deno `@rullama/knowledge` package keeps its client role and talks to
  a Rust RAG service over the existing `RagClient` interface.
...
`@rullama/mcp-server` (MCP server, IPC transport) or `@rullama/a2a` (Google A2A
protocol).
