...
describe("search index builder", () => {
  ...
  it("fuse_finds_expected_hit", () => {
    const corpus: IndexEntry[] = [
      {
        href: "/crates/rullama-cognition",
        title: "rullama-cognition",
        section: "Framework Crates",
        body: "Unified intelligence layer — knowledge graphs, adaptive prompting, RAG.",
      },
      {
        href: "/crates/rullama-storage",
        title: "rullama-storage",
        section: "Framework Crates",
    ...
    expect(hits[0].item.href).toBe("/crates/rullama-cognition");
    ...
    expect(bodyHits[0].item.href).toBe("/crates/rullama-cognition");
  });
});
