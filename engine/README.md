...
This is an **isolated sub-workspace** inside the [rullama](../) platform repo,
...
| Crate | What it is |
|---|---|
| [`rullama-engine`](rullama-engine) | The inference engine — model load/streaming, forward pass + WGSL kernels, sampling, KV cache, tokenizer/template, LoRA inference, vision/audio, diffusion, image-gen. |
| [`rullama-lora`](rullama-lora) | Local LoRA fine-tuning over the same wgpu kernels. Native + `wasm32`; exposes the `TrainingSession` wasm-bindgen surface the PWA's Fine-tune tab consumes. |
...
wasm-pack build rullama-engine --target web --release \
    --out-dir ../../pkg --out-name rullama
```
...
— see the `serve` bin in `rullama-engine`.
