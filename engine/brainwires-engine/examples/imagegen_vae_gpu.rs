...
fn main() {
    ...
    let dir = a.next().unwrap_or_else(|| "weights/Z-Image-Turbo/vae".to_string());
    let lsz: usize = a.next().and_then(|s| s.parse().ok()).unwrap_or(4);

    let cfg = VaeConfig::parse(&std::fs::read(format!("{dir}/config.json")).expect("config")).unwrap();
    let st = ShardedSafetensors::open_single(format!("{dir}/diffusion_pytorch_model.safetensors")).unwrap();
    let lc = cfg.latent_channels as usize;
    let latent: Vec<f32> = (0..lc * lsz * lsz).map(|i| ((i % 17) as f32 - 8.0) * 0.1).collect();

    let ctx = pollster::block_on(WgpuCtx::new()).expect("wgpu");
    let pipes = Pipelines::new(&ctx.device);

    println!("CPU decode [{lc},{lsz},{lsz}] → RGB [3,{},{}]...", lsz * 8, lsz * 8);
    let t0 = std::time::Instant::now();
    let cpu = VaeDecoder::new(&st, &cfg).decode(&latent, lsz, lsz).expect("cpu decode");
    let cpu_dt = t0.elapsed();

    println!("GPU decode...");
    let t1 = std::time::Instant::now();
    let gpu = pollster::block_on(VaeGpu::new(&ctx, &pipes, &st, &cfg).decode(&latent, lsz, lsz)).expect("gpu decode");
    ...
    println!("CPU {cpu_dt:.2?}  vs  GPU {gpu_dt:.2?}  ({:.1}× )", cpu_dt.as_secs_f64() / gpu_dt.as_secs_f64().max(1e-9));
