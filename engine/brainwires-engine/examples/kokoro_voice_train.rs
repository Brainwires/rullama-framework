...
const IDS: [i64; 25] = [0, 50, 83, 54, 156, 31, 3, 16, 50, 157, 39, 16, 69, 123, 16, 52, 63, 16, 62, 83, 46, 156, 24, 6, 0];
...
fn main() {
    ...
    let gguf = args.next().expect("usage: kokoro_voice_train <gguf> [iters]");
    ...
    let target_audio = pollster::block_on(model.synthesize_gpu_fast(&ctx, &pipes, &mut wc, &IDS, &target));
    let target_sig = voice_signature(&target_audio);

    // start from a perturbed voice vector (a deterministic offset from af_heart)
    let init: Vec<f32> = target.iter().enumerate().map(|(i, &v)| v + 0.25 * ((i as f32) * 0.7).sin()).collect();
    println!("init ||style - target|| = {:.4}", l2(&init, &target));

    let res = pollster::block_on(model.train_voice(&ctx, &pipes, &mut wc, &IDS, &target_sig, &init, iters, 0.06, 12345));

    println!("loss curve: {:?}", res.loss_curve.iter().map(|v| (v * 1e4).round() / 1e4).collect::<Vec<_>>());
    println!("loss {:.4e} -> {:.4e}  ({:.0}% reduction)", res.loss_curve[0], *res.loss_curve.last().unwrap(), 100.0 * (1.0 - res.loss_curve.last().unwrap() / res.loss_curve[0]));
    println!("recovered ||style - target|| = {:.4}  (init was {:.4})", l2(&res.style, &target), l2(&init, &target));
