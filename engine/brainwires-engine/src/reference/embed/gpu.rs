...
impl EmbedModel {
    ...
    pub async fn embed_ids_gpu(
        ...
        for layer in 0..cfg.n_layers {
            self.layer_gpu(ctx, pipes, &mut wc, layer, t, &mut hidden).await?;
        }
        ...
        let keep = if target_dim == 0 { projected.len() } else { target_dim.min(projected.len()) };
    ...
    async fn layer_gpu(
        ...
        for p in 0..t {
            rmsnorm(&hidden[p * d..(p + 1) * d], Some(&attn_norm), eps, &mut x[p * d..(p + 1) * d]);
        }

        // Q/K/V projections on GPU (batched over T).
        let q = self.gpu_matmul(ctx, pipes, wc, &format!("{prefix}attn_q.weight"), &x, d, n_heads * hd, t).await?;
        let k = self.gpu_matmul(ctx, pipes, wc, &format!("{prefix}attn_k.weight"), &x, d, n_kv * hd, t).await?;
        let v = self.gpu_matmul(ctx, pipes, wc, &format!("{prefix}attn_v.weight"), &x, d, n_kv * hd, t).await?;
        ...
        for p in 0..t {
            ...
            for h in 0..n_heads {
                rmsnorm(&q[p * n_heads * hd + h * hd..p * n_heads * hd + (h + 1) * hd], Some(&q_norm), eps, &mut qn[h * hd..(h + 1) * hd]);
            }
            ...
            for h in 0..n_kv {
                rmsnorm(&k[p * n_kv * hd + h * hd..p * n_kv * hd + (h + 1) * hd], Some(&k_norm), eps, &mut kn[h * hd..(h + 1) * hd]);
            }
        ...
        for qh in 0..n_heads {
            ...
            for i in 0..t {
                for j in 0..t {
                    let within = if cfg.causal { j <= i && (!is_swa || i - j < window) }
                        else if is_swa { i.abs_diff(j) < window } else { true };
                    if !within { scores[j] = f32::NEG_INFINITY; continue; }
                    let q_off = i * n_heads * hd + qh * hd;
                    let k_off = j * n_kv * hd + kvh * hd;
                    let mut acc = 0f32;
                    for dd in 0..hd { acc += q_all[q_off + dd] * k_all[k_off + dd]; }
                    scores[j] = acc * scale_f;
                }
                softmax(&mut scores);
                let out_off = i * n_heads * hd + qh * hd;
                for dd in 0..hd { ctx_attn[out_off + dd] = 0.0; }
                for j in 0..t {
                    let w = scores[j];
                    if w == 0.0 { continue; }
                    let v_off = j * n_kv * hd + kvh * hd;
                    for dd in 0..hd { ctx_attn[out_off + dd] += w * v[v_off + dd]; }
                }
            }
        }

        // output projection (GPU) + post-attn norm + residual (CPU).
        let attn_out = self.gpu_matmul(ctx, pipes, wc, &format!("{prefix}attn_output.weight"), &ctx_attn, n_heads * hd, d, t).await?;
        let post_attn = self.weights.load(&format!("{prefix}post_attention_norm.weight"))?;
        for p in 0..t {
            let mut h2 = vec![0f32; d];
            rmsnorm(&attn_out[p * d..(p + 1) * d], Some(&post_attn), eps, &mut h2);
            for k in 0..d { hidden[p * d + k] = h2[k] + residual[p * d + k]; }
        }
        ...
        for p in 0..t {
            rmsnorm(&hidden[p * d..(p + 1) * d], Some(&ffn_norm), eps, &mut xn[p * d..(p + 1) * d]);
        }
        let gate = self.gpu_matmul(ctx, pipes, wc, &format!("{prefix}ffn_gate.weight"), &xn, d, ffn_n, t).await?;
        let up = self.gpu_matmul(ctx, pipes, wc, &format!("{prefix}ffn_up.weight"), &xn, d, ffn_n, t).await?;
        let mut act = vec![0f32; t * ffn_n];
        for p in 0..t {
            geglu_split(&gate[p * ffn_n..(p + 1) * ffn_n], &up[p * ffn_n..(p + 1) * ffn_n], &mut act[p * ffn_n..(p + 1) * ffn_n]);
        }
        let mlp_out = self.gpu_matmul(ctx, pipes, wc, &format!("{prefix}ffn_down.weight"), &act, ffn_n, d, t).await?;
        let post_ffw = self.weights.load(&format!("{prefix}post_ffw_norm.weight"))?;
        for p in 0..t {
            let mut h3 = vec![0f32; d];
            rmsnorm(&mlp_out[p * d..(p + 1) * d], Some(&post_ffw), eps, &mut h3);
            for k in 0..d { hidden[p * d + k] = h3[k] + residual[p * d + k]; }
        }
        Ok(())
    }
    ...
    async fn gpu_matmul(
        ...
        let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("embed.mm") });
