...
impl<'a> GpuTts<'a> {
    pub fn new(m: &'a KokoroModel, ctx: &'a WgpuCtx, p: &'a Pipelines, wc: &'a mut WeightCache) -> Self {
        let dummy = make_dummy_storage(&ctx.device, "dummy");
        Self { m, ctx, p, wc, dummy, scratch: Vec::new() }
    }
    ...
    fn adain_resblk1d(&mut self, enc: &mut wgpu::CommandEncoder, x: &wgpu::Buffer, dim_in: usize, t: usize, dim_out: usize, upsample: bool, prefix: &str, style: &[f32]) -> (wgpu::Buffer, usize) {
        ...
        conv1d_chained(self.ctx, self.p, enc, &pool, &c1w, Some(&c1b), &self.dummy, &cv1, dim_in, t_pool, dim_out, 3, 1, 1, 1, 1);
        ...
        let sc = if learned_sc {
            ...
            conv1d_chained(self.ctx, self.p, enc, &short_in, &cw, None, &self.dummy, &out, dim_in, t_pool, dim_out, 1, 1, 0, 1, 1);
            out
        } else {
            short_in
        };
    ...
    fn adain_resblock1(&mut self, enc: &mut wgpu::CommandEncoder, x: &wgpu::Buffer, c: usize, t: usize, k: usize, dil: [usize; 3], prefix: &str, style: &[f32]) -> wgpu::Buffer {
        ...
        for j in 0..3 {
            ...
            let (h1, h2, h3, h4, h5, rb) = (self.alloc(c * t), self.alloc(c * t), self.alloc(c * t), self.alloc(c * t), self.alloc(c * t), self.alloc(c * t));
            ...
            conv1d_chained(self.ctx, self.p, enc, &h2, &c1w, Some(&c1b), &self.dummy, &h3, c, t, c, k, 1, pad1, dil[j], 1);
    ...
    fn generator(&mut self, enc: &mut wgpu::CommandEncoder, x: wgpu::Buffer, xt_len: usize, har: &wgpu::Buffer, har_len: usize, style: &[f32]) -> (wgpu::Buffer, usize) {
        ...
        for i in 0..rates.len() {
            ...
            let (xsrc, nres_k, ts) = if i + 1 < rates.len() {
                ...
                conv1d_chained(self.ctx, self.p, enc, har, &ncw, Some(&ncb), &self.dummy, &o, nfft + 2, har_len, cout, sf * 2, sf, (sf + 1) / 2, 1, 1);
                (o, 7usize, ts)
            } else {
                let o = self.alloc(cout * har_len);
                conv1d_chained(self.ctx, self.p, enc, har, &ncw, Some(&ncb), &self.dummy, &o, nfft + 2, har_len, cout, 1, 1, 0, 1, 1);
                (o, 11usize, har_len)
            };
            let xsrc = self.adain_resblock1(enc, &xsrc, cout, ts, nres_k, [1, 3, 5], &format!("k.decoder.generator.noise_res.{i}"), style);
            ...
            conv_transpose1d_chained(self.ctx, self.p, enc, &cur, &uw, Some(&ub), &self.dummy, &up0, cin, tcur, cout, kk, rates[i], (kk - rates[i]) / 2, 0, 1);
            ...
            for (j, (&rk, rd)) in rkernels.iter().zip(rdil.iter()).enumerate() {
                let rb = self.adain_resblock1(enc, &up, cout, tup, rk, [rd[0], rd[1], rd[2]], &format!("k.decoder.generator.resblocks.{}", i * rkernels.len() + j), style);
                residual_add_chained(self.ctx, self.p, enc, &acc, &rb, cout * tup);
            }
            scale_chained(self.ctx, self.p, enc, &acc, cout * tup, 1.0 / rkernels.len() as f32);
            cur = acc;
            cin = cout;
            tcur = tup;
        }
        ...
        conv1d_chained(self.ctx, self.p, enc, &cur, &cpw, Some(&cpb), &self.dummy, &post, cin, tcur, nfft + 2, 7, 1, 3, 1, 1);
        let tpost = tcur;
        ...
        istft_chained(self.ctx, self.p, enc, &spec, &phase, &audio, nbins, tpost, nfft, hop);
        (audio, out_len)
    }
    ...
    async fn read(&self, buf: &wgpu::Buffer, n: usize) -> Vec<f32> {
        ...
        let mut enc = self.ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("rd") });
        enc.copy_buffer_to_buffer(buf, 0, &read, 0, (n * 4) as u64);
        self.ctx.queue.submit(Some(enc.finish()));
        read_back_f32(&self.ctx.device, &read).await.expect("readback")
    }
}
...
impl KokoroModel {
    ...
    pub async fn synthesize_gpu_fast(&self, ctx: &WgpuCtx, p: &Pipelines, wc: &mut WeightCache, ids: &[i64], ref_s: &[f32]) -> Vec<f32> {
        ...
        let run_branch = |g: &mut GpuTts, which: &str| -> (wgpu::Buffer, usize) {
            let xb = g.up(&x_cm);
            let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("f0n") });
            let (h, t1) = g.adain_resblk1d(&mut enc, &xb, c, f, c, false, &format!("k.predictor.{which}.0"), prosodic);
            let (h, t2) = g.adain_resblk1d(&mut enc, &h, c, t1, half, true, &format!("k.predictor.{which}.1"), prosodic);
            let (h, t3) = g.adain_resblk1d(&mut enc, &h, half, t2, half, false, &format!("k.predictor.{which}.2"), prosodic);
            let pw = g.w(&format!("k.predictor.{which}_proj.weight"));
            let pb = g.w(&format!("k.predictor.{which}_proj.bias"));
            let out = g.alloc(t3);
            conv1d_chained(ctx, p, &mut enc, &h, &pw, Some(&pb), &g.dummy, &out, half, t3, 1, 1, 1, 0, 1, 1);
        ...
        let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("dec+gen") });
        let asr_b = g.up(&asr);
        let f0_b = g.up(&f0);
        let n_b = g.up(&n);
        ...
        conv1d_chained(ctx, p, &mut enc, &f0_b, &f0w, Some(&f0cb), &g.dummy, &f0d, 1, fl2, 1, 3, 2, 1, 1, 1);
        let nd = g.alloc(f0d_t);
        conv1d_chained(ctx, p, &mut enc, &n_b, &nw, Some(&ncb), &g.dummy, &nd, 1, fl2, 1, 3, 2, 1, 1, 1);
        let cat0 = g.concat(&mut enc, &[(&asr_b, c), (&f0d, 1), (&nd, 1)], ff);
        let (dec_encode, _) = g.adain_resblk1d(&mut enc, &cat0, c + 2, ff, 1024, false, "k.decoder.encode", timbre);
        let arw = g.w("k.decoder.asr_res.0.weight");
        let arb = g.w("k.decoder.asr_res.0.bias");
        let asr_res = g.alloc(64 * ff);
        conv1d_chained(ctx, p, &mut enc, &asr_b, &arw, Some(&arb), &g.dummy, &asr_res, c, ff, 64, 1, 1, 0, 1, 1);
        let mut x = dec_encode;
        let mut tcur = ff;
        let mut xc = 1024usize;
        for i in 0..4 {
            let dim_in = xc + 64 + 2;
            let xin = g.concat(&mut enc, &[(&x, xc), (&asr_res, 64), (&f0d, 1), (&nd, 1)], tcur);
            let upsample = i == 3;
            let dim_out = if i < 3 { 1024 } else { 512 };
            let (nx, nt) = g.adain_resblk1d(&mut enc, &xin, dim_in, tcur, dim_out, upsample, &format!("k.decoder.decode.{i}"), timbre);
            x = nx;
            tcur = nt;
            xc = dim_out;
        }
