...
use super::{adaptive_avg_pool2d_1, Conv, Map, ResBlk, LRELU};
...
fn take(w: &W, name: &str) -> Vec<f32> {
    w.get(name).unwrap_or_else(|| panic!("missing style-encoder weight: {name}")).clone()
}
...
impl StyleEncoder {
    ...
    pub fn from_weights(w: &W, prefix: &str) -> Self {
        ...
        let conv0 = Conv {
            ...
            oc: 64, kh: 3, kw: 3, stride: 1, pad: 1, groups: 1,
        };
        let blks = (0..4)
            .map(|i| {
                ...
                ResBlk {
                    conv1: Conv { w: take(w, &b("conv1.weight")), b: Some(take(w, &b("conv1.bias"))), oc: din, kh: 3, kw: 3, stride: 1, pad: 1, groups: 1 },
                    down: Conv { w: take(w, &b("down.weight")), b: Some(take(w, &b("down.bias"))), oc: din, kh: 3, kw: 3, stride: 2, pad: 1, groups: din },
                    conv2: Conv { w: take(w, &b("conv2.weight")), b: Some(take(w, &b("conv2.bias"))), oc: dout, kh: 3, kw: 3, stride: 1, pad: 1, groups: 1 },
                    sc: (din != dout).then(|| Conv { w: take(w, &b("sc.weight")), b: None, oc: dout, kh: 1, kw: 1, stride: 1, pad: 0, groups: 1 }),
                }
            })
            .collect();
        let conv_out = Conv {
            ...
            oc: 512, kh: 5, kw: 5, stride: 1, pad: 0, groups: 1,
        };
        Self { conv0, blks, conv_out, lin_w: take(w, &p("linear.weight")), lin_b: take(w, &p("linear.bias")) }
    }
