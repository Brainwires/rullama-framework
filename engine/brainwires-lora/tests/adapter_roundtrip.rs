...
fn adapter_f32_round_trip_is_bit_identical() {
    ...
    state.insert(LoraKey::new(0, "attn_k"), 8, 2, 4, 4.0, 2).unwrap();

    {
        let layer = state.get(&LoraKey::new(0, "attn_q")).unwrap();
        ctx.queue.write_buffer(&layer.a, 0, bytemuck::cast_slice(&A_VALS));
        ctx.queue.write_buffer(&layer.b, 0, bytemuck::cast_slice(&B_VALS));
    }
    ctx.device
    ...
    assert_eq!(a_round, A_VALS.to_vec(), "f32 A round-trip must be bit-identical");
    assert_eq!(b_round, B_VALS.to_vec(), "f32 B round-trip must be bit-identical");
}
...
fn adapter_f16_round_trip_within_quantisation_tolerance() {
    ...
    for (orig, round) in A_VALS.iter().zip(a_round.iter()) {
        assert!((orig - round).abs() < 1e-3, "A f16 round-trip drift too large: {orig} vs {round}");
    }
    for (orig, round) in B_VALS.iter().zip(b_round.iter()) {
        assert!((orig - round).abs() < 1e-3, "B f16 round-trip drift too large: {orig} vs {round}");
    }
}
