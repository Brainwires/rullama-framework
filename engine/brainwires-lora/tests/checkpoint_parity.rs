...
fn checkpoint_parity_matches_standard_grads() {
    ...
    pollster::block_on(async move {
        ...
        assert_eq!(std_grads.len(), ckpt_grads.len(), "different LoRA layer count");
...
async fn run_one_step(
    ...
    let model = Model::load_native(gguf.to_vec())
        .await
        .expect("load model");
    ...
    hp.seed = 0xC0FFEE;
    hp.gradient_checkpointing = gradient_checkpointing;
    hp.max_grad_norm = 0.0;
