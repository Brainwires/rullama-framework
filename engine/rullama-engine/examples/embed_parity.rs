...
//!
//! Usage:
//!   cargo run -p rullama-engine --release --example embed_parity -- \
...
use rullama_engine::reference::embed::EmbedModel;
use rullama_engine::tokenizer::SpmTokenizer;
...
fn main() -> ExitCode {
    ...
    if env::var("RULLAMA_EMBED_GPU").is_ok() {
        let gpu = pollster::block_on(async {
            use std::sync::Arc;
            let ctx = rullama_engine::backend::WgpuCtx::new().await?;
            let pipes = rullama_engine::backend::Pipelines::new(&ctx.device);
            let wcache = rullama_engine::backend::WeightCache::new(
