...
use rullama_engine::gguf::{GgufReader, GgufValue};
    ...
    match Gemma4Config::from_gguf(&r) {
        Ok(c) => {
            ...
            let swa = c
                .layer_kinds
                .iter()
                .filter(|k| matches!(k, rullama_engine::model::config::LayerKind::SlidingWindow))
                .count();
            ...
            for i in 0..c.n_layers.min(12) {
                let k = match c.kind(i) {
                    rullama_engine::model::config::LayerKind::SlidingWindow => "S",
                    rullama_engine::model::config::LayerKind::Global => "G",
                };
                print!("{}{} ", k, c.ffn(i));
            }
            if c.n_layers > 12 {
