// Bench harness dispatches different matmul variants — each call mirrors
// the kernel's uniform layout.
...
use rullama_engine::backend::{Pipelines, WgpuCtx, dispatch};
...
fn run_shape(
    ...
    for _ in 0..3 {
        ...
        dispatch::matmul_f16_batched_chained(
...
fn run_shape_force(
    ...
    let _ = batch;
    println!(
        "{label:30}                                       : {per_iter:?}/iter   {gflops:.2} GFLOPS"
    );
}
...
fn main() {
    ...
    println!(
        "Adapter: {} / {:?}  (subgroups: {})",
        info.name, info.backend, ctx.has_subgroups
    );
    ...
    run_shape_force(
        ...
        &ctx,
        &pipes,
        &pipes.f16_matmul_batched_tiled,
        &bg_v1,
        (n as u32).div_ceil(8),
        (batch as u32).div_ceil(8),
        k,
        n,
        batch,
        5,
    ...
    run_shape_force(
        ...
        &ctx,
        &pipes,
        &pipes.f16_matmul_batched_tiled_v2,
        &bg_v2,
    ...
    run_shape_force(
        ...
        &ctx,
        &pipes,
        &pipes.f16_matmul_batched_tiled_v3,
        &bg_v3,
        (n as u32).div_ceil(32),
        (batch as u32).div_ceil(32),
        k,
        n,
        batch,
        5,
    ...
    run_shape_force(
        ...
        &ctx,
        &pipes,
        &pipes.f16_matmul_batched_tiled_v4,
        &bg_v4,
        (n as u32).div_ceil(32),
    ...
    ctx.queue
        .write_buffer(&q_buf, 0, bytemuck::cast_slice(&zeros));
    ctx.queue
        .write_buffer(&k_buf, 0, bytemuck::cast_slice(&zeros));
    ctx.queue
        .write_buffer(&v_buf, 0, bytemuck::cast_slice(&zeros));
    ...
    for _ in 0..2 {
        let mut enc = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        dispatch::vision_attention_chained(
            &ctx, &pipes, &mut enc, &q_buf, &k_buf, &v_buf, &out_buf, head_dim, n_heads, n_patches,
        );
        ctx.queue.submit(Some(enc.finish()));
        ctx.device
            .poll(wgpu::PollType::Wait {
    ...
    let per_iter = elapsed / n_iters as u32;
    println!(
        "vision_attention (router → Q8): {per_iter:?}/iter  (×16 blocks ≈ {:?} total)",
        per_iter * 16
    );
    ...
    for _ in 0..2 {
        let mut enc = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        dispatch::vision_attention_flash_chained(
            &ctx, &pipes, &mut enc, &q_buf, &k_buf, &v_buf, &out_buf, head_dim, n_heads, n_patches,
        );
        ctx.queue.submit(Some(enc.finish()));
        ctx.device
    ...
    println!(
        "vision_attention_flash (Q=1)   : {per_iter2:?}/iter  (×16 blocks ≈ {:?} total)",
        per_iter2 * 16
    );
    ...
    for _ in 0..2 {
        let mut enc = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        dispatch::vision_attention_flash_q8_chained(
            &ctx, &pipes, &mut enc, &q_buf, &k_buf, &v_buf, &out_buf, head_dim, n_heads, n_patches,
        );
        ctx.queue.submit(Some(enc.finish()));
        ctx.device
    ...
    println!(
        "vision_attention_flash (Q=8)   : {per_iter3:?}/iter  (×16 blocks ≈ {:?} total)",
        per_iter3 * 16
    );
    ...
    for _ in 0..2 {
        let mut enc = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        dispatch::vision_attention_flash_q16_chained(
            &ctx, &pipes, &mut enc, &q_buf, &k_buf, &v_buf, &out_buf, head_dim, n_heads, n_patches,
        );
        ctx.queue.submit(Some(enc.finish()));
        ctx.device
    ...
    println!(
        "vision_attention_flash (Q=16)  : {per_iter4:?}/iter  (×16 blocks ≈ {:?} total)",
        per_iter4 * 16
    );
    ...
    if let Some(sub_hpd) = pipes.vision_attention_flash_sub_hpd.as_ref() {
        let q_t = ctx.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("q_t"),
            size: (n_patches * n_heads * head_dim * 4) as u64,
        ...
        for _ in 0..2 {
            let mut enc = ctx
                .device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
            dispatch::transpose_phd_to_hpd_chained(
                &ctx, &pipes, &mut enc, &q_buf, &q_t, n_patches, n_heads, head_dim,
            );
            dispatch::transpose_phd_to_hpd_chained(
                &ctx, &pipes, &mut enc, &k_buf, &k_t, n_patches, n_heads, head_dim,
            );
            dispatch::transpose_phd_to_hpd_chained(
                &ctx, &pipes, &mut enc, &v_buf, &v_t, n_patches, n_heads, head_dim,
            );
            dispatch::vision_attention_flash_sub_hpd_chained(
                &ctx, &pipes, sub_hpd, &mut enc, &q_t, &k_t, &v_t, &out_t, head_dim, n_heads,
                n_patches,
            );
            dispatch::transpose_hpd_to_phd_chained(
                &ctx, &pipes, &mut enc, &out_t, &out_buf, n_patches, n_heads, head_dim,
            );
            ctx.queue.submit(Some(enc.finish()));
            ctx.device
                .poll(wgpu::PollType::Wait {
        ...
        println!(
            "vision_attention_flash (sub+HPD): {per_iterh:?}/iter  (with 3+1 transposes; ×16 ≈ {:?})",
            per_iterh * 16
        );
        println!(
            "vision_attention_flash (HPD only): {per_itera:?}/iter  (attn-only; ×16 ≈ {:?})",
            per_itera * 16
        );
        ...
        if let Some(sub_hpd_f16_q16) = pipes.vision_attention_flash_sub_hpd_f16_q16.as_ref() {
            for _ in 0..2 {
                let mut enc = ctx
                    .device
                    .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
                dispatch::vision_attention_flash_sub_hpd_f16_q16_chained(
                    &ctx,
                    &pipes,
                    sub_hpd_f16_q16,
                    &mut enc,
            ...
            println!(
                "vision_attention_flash (HPD f16 Q=16): {per_iteraq:?}/iter  (attn-only; ×16 ≈ {:?})",
                per_iteraq * 16
            );
        }
        ...
        if let Some(sub_hpd_f16) = pipes.vision_attention_flash_sub_hpd_f16.as_ref() {
            ...
            println!(
                "vision_attention_flash (HPD f16): {per_iteraf:?}/iter  (attn-only; ×16 ≈ {:?})",
                per_iteraf * 16
            );
        }
    }
    ...
    if let Some(sub64) = pipes.vision_attention_flash_sub_t64.as_ref() {
        for _ in 0..2 {
            let mut enc = ctx
                .device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
            dispatch::vision_attention_flash_sub_t64_chained(
                &ctx, &pipes, sub64, &mut enc, &q_buf, &k_buf, &v_buf, &out_buf, head_dim, n_heads,
                n_patches,
            );
            ctx.queue.submit(Some(enc.finish()));
            ctx.device
        ...
        println!(
            "vision_attention_flash (T64/Q12): {per_iter6:?}/iter  (×16 blocks ≈ {:?} total)",
            per_iter6 * 16
        );
    }
    ...
    if let Some(sub) = pipes.vision_attention_flash_subgroup.as_ref() {
        for _ in 0..2 {
            let mut enc = ctx
                .device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
            dispatch::vision_attention_flash_subgroup_chained(
                &ctx, &pipes, sub, &mut enc, &q_buf, &k_buf, &v_buf, &out_buf, head_dim, n_heads,
                n_patches,
            );
            ctx.queue.submit(Some(enc.finish()));
            ctx.device
        ...
        println!(
            "vision_attention_flash (sub)   : {per_iter5:?}/iter  (×16 blocks ≈ {:?} total)",
            per_iter5 * 16
        );
    }
    ...
    let mk_buf = |label: &'static str, n: usize| {
        ctx.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some(label),
            size: (n * 4) as u64,
            usage: wgpu::BufferUsages::STORAGE
    ...
    bench("rmsnorm_per_row hidden", &|enc| {
        dispatch::rmsnorm_per_row_chained(
            &ctx,
            &pipes,
            enc,
            &scratch_hidden,
    ...
    bench("rmsnorm_per_row Q/K (per-head)", &|enc| {
        dispatch::rmsnorm_per_row_chained(
            ...
            &scratch_norm,
            &scratch_qk2,
            n_patches_v * n_heads_v,
            head_dim_v,
            1e-6,
        );
    });
    bench("clamp on ffn buf (7M)", &|enc| {
        dispatch::clamp_chained(
            &ctx,
            &pipes,
            enc,
            &scratch_ffn,
