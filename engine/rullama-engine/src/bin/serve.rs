//! `rullama-serve` — an OpenAI-compatible HTTP front end that hosts one GGUF
...
async fn main() {
    ...
    let mut model_name = "rullama-engine".to_string();
    while let Some(a) = args.next() {
        match a.as_str() {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "usage: rullama-serve <gguf-path> [--port N] [--host ADDR] [--model-name NAME]"
                );
                return;
            }
    ...
    let Some(path) = path else {
        ...
        eprintln!("usage: rullama-serve <gguf-path> [--port N] [--host ADDR] [--model-name NAME]");
        std::process::exit(2);
    };
    ...
    eprintln!("rullama-serve: {model_name} on http://{addr}/v1");
