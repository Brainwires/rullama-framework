// bf16 v3 matmul with f16 workgroup storage + f16 inner-loop arithmetic.
// BF16 analogue of f16_matmul_batched_tiled_v3_f16lds.wgsl — same trick,
// different global-side unpack.
//
// Numerical caveat: BF16 stores values up to ~3.4e38 (8-bit exponent, same
// as f32), while f16 caps at ~6.5e4 (5-bit exponent). Conformer activations
// and weights stay well within f16 range in practice, but downcasting
// loses dynamic range. Accumulators stay f32.
//
...
fn main(
    ...
    for (var iter: u32 = 0u; iter < n_k_iters; iter = iter + 1u) {
        ...
        // Load w: bf16-packed u32 → f32 (via bitcast) → f16 LDS.
        for (var s: u32 = 0u; s < 4u; s = s + 1u) {
            ...
            if (wcol < params.n && pi < half_k) {
                ...
                w_tile[nw * TILE_K + 2u * kp]      = f16(lo);
                w_tile[nw * TILE_K + 2u * kp + 1u] = f16(hi);
            } else {
