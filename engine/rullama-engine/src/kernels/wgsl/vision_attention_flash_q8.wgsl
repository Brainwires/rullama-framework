// Multi-query flash vision attention with Q = 8. Doubles the queries-per-WG
// over the Q=4 variant, halving workgroup launch count and K/V global bandwidth
// again. Uses function-scope arrays for the per-query online-softmax state
// (m, l, o) so we don't need an 8-case switch statement for register access.
//
// I/O contract identical to vision_attention.wgsl.
//
// Dispatch:  (ceil(n_patches / 8), n_heads, 1)
// Workgroup: 64 threads.
//
// Workgroup storage:
//   q_shared    (8 × HEAD_DIM_MAX = 512 f32)     2 KB
//   kv_tile     (TILE_T × HEAD_DIM_MAX = 2048)   8 KB
//   tile_scores (8 × WG = 512 f32)               2 KB
//   rbuf        (WG)                           256 B
//   sum_buf     (WG)                           256 B
//   --------------------------------
//   total                                    ~12.5 KB
...
fn main(
    ...
    // Per-query state in function-scope arrays. WGSL's naga lowers this to
    // either registers (if the compiler can prove indices stay constant after
    // unrolling) or a private-memory backing store. Either way, no expensive
    // switch statement.
    ...
    for (var tile: u32 = 0u; tile < n_tiles; tile = tile + 1u) {
        ...
        // Per-query score + reduction + softmax merge.
