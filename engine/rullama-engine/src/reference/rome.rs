...
//!     from `rullama_lora::scratch::LayerActivations` only in
...
/// `rullama_lora::scratch::TrainingScratch`'s scratch fields — we
/// duplicate the allocation here rather than depend on
/// `rullama-lora` (cycle: rullama-lora already depends on
/// rullama).
