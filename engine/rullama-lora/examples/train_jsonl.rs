//! Train rullama-lora on a JSONL dataset.
//!
...
//!
//! ```text
//! cargo run -p rullama-lora --example train_jsonl --release -- \
//!     /path/to/gemma4-e2b.gguf \
//!     crates/rullama-lora/examples/data/echo.jsonl
//! ```
//!
...
use rullama_lora::dataset_loader::TrainingDataset;
use rullama_lora::shared::config::{LoraConfig, LossMode, LrScheduler, TrainingHyperparams};
...
async fn run() -> Result<(), BoxError> {
    ...
    if let Some(path) = &resume_path
        && path.exists()
    {
        let n = rullama_lora::load_adapter_into_state(session.lora_state_mut(), path)
