...
use rullama_engine::backend::dispatch::{
    AdamConfig, adam_step_chained, scale_chained, sum_of_squares_chained,
};
use rullama_engine::reference::forward_chained::{
...
fn lora_projection_dims(
    cfg: &rullama_engine::model::config::Gemma4Config,
    layer: u32,
    proj: &str,
...
fn build_lora_state(
    ctx: Arc<rullama_engine::backend::WgpuCtx>,
    cfg: &rullama_engine::model::config::Gemma4Config,
    lora_cfg: &LoraConfig,
    seed_base: u64,
...
fn reject_qat_base(model: &Model) -> Result<(), TrainingError> {
    if let Ok(dt) = model.forward().wcache().dtype("blk.0.attn_q.weight")
        && matches!(
            dt,
            rullama_engine::gguf::GgmlDtype::Q4_0 | rullama_engine::gguf::GgmlDtype::Q8_0
        )
    {
    ...
    pub async fn forward_backward_per_position_with_progress<'cb>(
        ...
        if memlog {
            eprintln!(
                ...
                input_ids.len(),
                n_active,
                rullama_engine::backend::gpu_mem::breakdown_str()
            );
        }
