...
cargo build -p rullama-engine --release --example rome_edit --example compute_rome_covariance 2>&1 | tail -2
...
for LAYER in $LAYERS; do
    ...
    if [ ! -f "$COV_PATH" ]; then
        ...
        if ! cargo run -p rullama-engine --release --example compute_rome_covariance -- \
