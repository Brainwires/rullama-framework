...
| Piece                  | Crate                       | Role                                         |
| ---------------------- | --------------------------- | -------------------------------------------- |
| `OllamaProvider`       | `rullama-provider`       | local LLM backend                            |
| `BudgetProvider`       | `rullama-call-policy`    | hard cap on tokens / spend / rounds          |
| `WebTool`              | `rullama-tool-builtins`  | `fetch_url` for HTTP GETs                    |
| `BuiltinToolExecutor`  | `rullama-tool-builtins`  | dispatches tool calls from the LLM           |
| `AgentBuilder`         | `rullama-inference`      | builds the `ChatAgent` with all of the above |

## Run

```bash
# defaults: http://localhost:11434 + gemma4:e2b
cargo run -p rullama-web-search-agent -- "what is the capital of France?"

# override host / model
OLLAMA_BASE_URL=http://my-host:11434 \
OLLAMA_DEFAULT_MODEL=llama3:8b \
    cargo run -p rullama-web-search-agent -- "summarize https://example.com"
```
...
- A real search engine — `fetch_url` only retrieves a URL the model already
  ...
  web search belongs in `rullama-rag`.
