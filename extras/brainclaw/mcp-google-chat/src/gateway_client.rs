...
//!
//! This is an intentional copy of the same pattern used by `mcp-discord`
//! and `mcp-slack`. Future refactors may extract a shared crate; we keep
//! adapters independent for now so each channel can evolve at its own pace.
...
impl GatewayClient {
    ...
    /// Forward events to the gateway and write outbound messages to
    /// Google Chat. Loops until either side closes.
    pub async fn run(
        mut self,
        mut event_rx: mpsc::Receiver<ChannelEvent>,
        chan: Arc<GoogleChatChannel>,
    ) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn handshake_fields() {
        let caps = ChannelCapabilities::RICH_TEXT | ChannelCapabilities::MENTIONS;
        let hs = build_handshake("tok", caps);
        assert_eq!(hs.channel_type, "google_chat");
        assert_eq!(hs.auth_token, "tok");
    ...
    #[test]
    fn backoff_starts_at_2s_minimum() {
        let d = backoff_next(Duration::from_millis(0));
