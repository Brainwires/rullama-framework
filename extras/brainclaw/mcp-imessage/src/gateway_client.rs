...
//!
//! Structural copy of `mcp-google-chat::gateway_client` — kept per-adapter
//! so each channel can evolve at its own pace.
...
use crate::imessage::ImessageChannel;
...
impl GatewayClient {
    ...
    /// BlueBubbles. Loops until either side closes.
    pub async fn run(
        mut self,
        mut event_rx: mpsc::Receiver<ChannelEvent>,
        chan: Arc<ImessageChannel>,
    ) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn handshake_fields() {
        let caps = ChannelCapabilities::REACTIONS;
        let hs = build_handshake("tok", caps);
        assert_eq!(hs.channel_type, "imessage");
