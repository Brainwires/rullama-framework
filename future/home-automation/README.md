...
- `brainwires-homeauto/` — the protocol stack lifted from `crates/brainwires-hardware/src/homeauto/`. Features: `zigbee`, `zwave`, `thread`, `matter`, `matter-ble`, `all`.
- `matter-tool/` — Matter commissioner / control CLI, lifted from `extras/matter-tool/`.
...
cargo test -p brainwires-homeauto --features matter --test matter_integration
```
