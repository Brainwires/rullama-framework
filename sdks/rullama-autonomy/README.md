...
- **GPIO Hardware Access** — re-exported from [`rullama-hardware`](../rullama-hardware) — safe GPIO pin management with allow-lists and auto-release for embedded/IoT
...
| `gpio` | GPIO hardware access via `rullama-hardware` (Linux) |
...
```rust
use rullama_autonomy::eval::{entity_importance_suite, multi_factor_suite};
...
cargo run -p rullama-autonomy --example safety_guard
cargo run -p rullama-autonomy --example health_monitor
cargo run -p rullama-autonomy --example session_metrics

# Self-improvement
cargo run -p rullama-autonomy --example self_improve_strategies --features self-improve
cargo run -p rullama-autonomy --example crash_recovery --features self-improve

# Git workflow & CI/CD
cargo run -p rullama-autonomy --example git_workflow_pipeline --features git-workflow
cargo run -p rullama-autonomy --example cicd_orchestrator --features webhook

# Environment interaction
cargo run -p rullama-autonomy --example cron_scheduler --features scheduler
cargo run -p rullama-autonomy --example fs_reactor --features reactor
cargo run -p rullama-autonomy --example service_manager --features services
```
...
- **GPIO**: empty allow-list by default (no pins accessible), auto-release on agent timeout — see [`rullama-hardware`](../rullama-hardware) for GPIO examples
