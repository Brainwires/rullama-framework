...
pub struct AutonomyConfig {
    ...
    /// [`rullama_hardware::gpio::config::GpioConfig`] (which the GPIO
...
fn default_webhook_log_dir() -> String {
    dirs::home_dir()
        ...
        .unwrap_or_else(|| "/tmp/rullama/webhook-logs".to_string())
}
    ...
    fn default() -> Self {
        Self {
            ...
            state_file: dirs::home_dir()
                ...
                .unwrap_or_else(|| "/tmp/rullama/crash-recovery.json".to_string()),
            enabled: true,
        }
    }
}
...
/// [`rullama_hardware::gpio::config::GpioConfig`].
///
/// Previously `rullama-autonomy` declared its own divergent
/// `GpioConfig` struct here, which (despite identical fields) was not
/// accepted by [`rullama_hardware::gpio::GpioPinManager::from_config`]
/// or [`rullama_hardware::gpio::GpioSafetyPolicy::from_config`]. The
/// re-export ensures `AutonomyConfig.gpio` is the same type the hardware
/// crate's pin manager and safety policy operate on.
#[cfg(feature = "gpio")]
pub use rullama_hardware::gpio::config::GpioConfig;
