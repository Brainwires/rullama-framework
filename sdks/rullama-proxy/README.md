...
[![Crates.io](https://img.shields.io/crates/v/rullama-proxy.svg)](https://crates.io/crates/rullama-proxy)
[![Documentation](https://img.shields.io/docsrs/rullama-proxy)](https://docs.rs/rullama-proxy)
[![License](https://img.shields.io/crates/l/rullama-proxy.svg)](LICENSE)

Protocol-agnostic proxy framework for debugging and transforming application traffic.

## Overview

`rullama-proxy` is a composable, async-first proxy framework built on Tokio. It supports multiple transport protocols, pluggable middleware, format conversion, and live traffic inspection — all behind a fluent builder API.
...
```rust
use rullama_proxy::middleware::auth::{AuthLayer, AuthStrategy};
...
```rust
use rullama_proxy::middleware::rate_limit::RateLimitLayer;
...
```rust
use rullama_proxy::middleware::header_inject::{HeaderInjectLayer, HeaderRule};
    ...
    .layer(
        HeaderInjectLayer::new()
            .set_request_header(
                ...
                HeaderValue::from_static("rullama-proxy"),
            )
...
```rust
use rullama_proxy::middleware::auth::AuthLayer;
...
```rust
use rullama_proxy::prelude::*;
use rullama_proxy::transport::InboundConnection;
