...
//! rullama-proxy framework. Based on the `claude-adapter` TypeScript project.
//!
//! ## Usage
//!
//! ```sh
//! cargo run -p rullama-proxy --example anthropic_openai_proxy --features http -- \
//!   --config ./config.json
//! ```
//!
//! ## Config format
...
async fn main() -> anyhow::Result<()> {
    ...
    eprintln!("anthropic-openai-proxy (rullama-proxy)");
