...
use rullama_proxy::inspector::broadcast::EventBroadcaster;
use rullama_proxy::inspector::store::EventStore;
use rullama_proxy::inspector::{EventDirection, TrafficEvent, TrafficEventKind};
use rullama_proxy::middleware::header_inject::HeaderInjectLayer;
use rullama_proxy::middleware::rate_limit::RateLimitLayer;
use rullama_proxy::middleware::{LayerAction, MiddlewareStack, ProxyLayer};
...
async fn header_inject_adds_header() {
    let layer = HeaderInjectLayer::new().set_request_header(
        ...
        HeaderValue::from_static("rullama"),
    );

    let action = layer.on_request(make_request()).await.unwrap();
    match action {
        LayerAction::Forward(req) => {
            assert_eq!(req.headers.get("x-test").unwrap(), "rullama");
        }
