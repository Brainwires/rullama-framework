...
//!
//! ```js
//! import init, { version, validate_message, serialize_history } from 'rullama-wasm';
//!
//! await init();
...
/// Re-export of the [`rullama_core`] crate for Rust consumers who depend on this
...
///
/// This is the version of the `rullama-wasm` package as declared in `Cargo.toml`.
///
/// # JS Example
///
/// ```js
...
///
/// Parses the input JSON string into a [`rullama_core::Message`] struct and
...
///
/// Parses the input JSON string into a [`rullama_core::Tool`] struct and
...
///
/// Takes a JSON array of [`rullama_core::Message`] objects and converts them
...
pub fn serialize_history(messages_json: &str) -> Result<String, String> {
    let messages: Vec<rullama_core::Message> =
        serde_json::from_str(messages_json).map_err(|e| format!("Invalid messages JSON: {e}"))?;
    let history = rullama_core::serialize_messages_to_stateless_history(&messages);
