//! Integration tests for all seven MCP tools exposed by `rullama-brain-server`.
//!
...
async fn search_memory_finds_captured_thought() -> Result<()> {
    ...
    let capture_resp = call_tool_json(
        ...
        "capture_thought",
        json!({
            "content": "rullama-test-magic-word appears exactly here",
        }),
    )
    .await?;
    let id = capture_resp["id"]
...
// Fuller PKS/BKS coverage belongs in `rullama-knowledge`'s own test suite.
