...
```bash
cargo run -p rullama-issues -- serve
```
...
{
  "rullama-issues": {
    "command": "/path/to/rullama-issues",
    "args": ["serve"]
  }
}
```
...
```json
{
  "rullama-issues": {
    "command": "cargo",
    "args": ["run", "-p", "rullama-issues", "--", "serve"]
  }
}
```
...
Issues and comments are persisted in **LanceDB** (embedded, no external server required) using the `rullama-storage` backend-agnostic layer. The default database path follows the platform convention set by `rullama-storage`.
