//! Integration tests for the MCP tools exposed by `rullama-issues`.
//!
...
/// `rullama-issues` tool returns a pretty-printed JSON string.
...
async fn search_finds_matching_issue() -> Result<()> {
    ...
    let target = call_tool_json(
        ...
        json!({
            ...
            "description": "The analytics rullama-search-needle widget overlaps the header.",
        }),
    )
    .await?;
    ...
    let resp = call_tool_json(
        &client,
        "search_issues",
        json!({ "query": "rullama-search-needle", "limit": 10 }),
    )
    .await?;
