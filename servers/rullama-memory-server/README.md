...
Storage is delegated to [`rullama-knowledge`](../../crates/rullama-knowledge) (LanceDB-backed thought store with semantic search). Tenant isolation is enforced on every request via `user_id`, which is passed through to the knowledge layer as its `owner_id` scope — a request for user A can never see, update, or delete data belonging to user B.
...
```sh
# Run with default settings (localhost:8765, ~/.local/share/rullama/memory/)
cargo run --bin rullama-memory-server
...
This service does not ship with bearer-token auth. If you need authentication, layer it via a reverse proxy (Traefik, Caddy, nginx, etc.) that validates credentials before forwarding to `rullama-memory`.
...
| `MEMORY_DB` | `~/.local/share/rullama/memory` | Knowledge storage directory (contains `brain.lance/`, `pks.db`, `bks.db`) |
| `RUST_LOG` | `rullama_memory_service=info` | Log filter |
...
│  POST /v1/memories/search   ─────┼──► BrainClient (rullama-knowledge)
│  PATCH/DELETE /v1/memories/{id}  │    └─► LanceDB (vectors)
│  GET  /health                    │    └─► SQLite (PKS/BKS)
└──────────────────────────────────┘
```

The server is a thin adapter over `rullama-knowledge` — all storage decisions (embedding model, vector index, backend selection) are made by the knowledge crate. Tenant isolation is implemented there, via the `owner_id` column on every thought row.
