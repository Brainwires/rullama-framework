//! rullama-memory-server — Mem0-compatible memory REST server backed by
//! `rullama-knowledge`.
//!
...
//!                  (default: ~/.local/share/rullama/memory)
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            std::env::var("RUST_LOG")
                .unwrap_or_else(|_| "rullama_memory_server=info,tower_http=debug".to_string()),
        )
        .init();
    ...
    tracing::info!(
        "rullama-memory-server storage directory: {}",
        storage_dir.display()
    );
    ...
    tracing::info!("rullama-memory-server listening on http://{addr}");
