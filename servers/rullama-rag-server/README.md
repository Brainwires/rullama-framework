...
This is the executable wrapper for `rullama-knowledge::rag` (formerly the standalone `rullama-rag` crate, absorbed in the 0.10 consolidation) that exposes all indexing, search, and code navigation functionality as MCP tools and slash commands.

## Quick Start

```bash
cargo run -p rullama-rag-server -- serve
```
...
- **Library**: Use `rullama-knowledge` crate (feature `rag`) in your Rust code for programmatic access
