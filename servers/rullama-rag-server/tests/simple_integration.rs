...
use anyhow::Result;
use rullama_rag::rag::config::Config;
use rullama_rag::rag::types::{
    AdvancedSearchRequest, ClearRequest, QueryRequest, StatisticsRequest,
};
use rullama_rag_server::mcp_server::RagMcpServer;
use std::path::Path;
...
fn write_rust_fixture(root: &Path) -> Result<()> {
    ...
    std::fs::write(
        ...
        "/// Returns the answer to life, the universe, and everything.\n\
         pub fn rullama_rag_magic_word_42() -> u32 { 42 }\n",
    )?;
    Ok(())
}
...
async fn test_index_then_query_returns_hit() -> Result<()> {
    ...
    let response = server
        ...
        .query_codebase(QueryRequest {
            query: "rullama_rag_magic_word_42 function returning 42".to_string(),
            path: None,
    ...
    let any_match = response
        .results
        .iter()
        .any(|r| r.content.contains("rullama_rag_magic_word_42"));
    assert!(
        any_match,
...
async fn test_search_by_filters_restricts_to_extension() -> Result<()> {
    ...
    std::fs::write(
        src_dir.join("foo.rs"),
        "/// Rust-side marker.\n\
         pub fn rullama_filter_marker_rust() -> u32 { 7 }\n",
    )?;
    std::fs::write(
        src_dir.join("foo.py"),
        "# Python-side marker.\n\
         def rullama_filter_marker_python():\n    return 7\n",
    )?;
    ...
    let rust_only = server
        ...
        .search_with_filters(AdvancedSearchRequest {
            query: "rullama_filter_marker function returning 7".to_string(),
            path: None,
