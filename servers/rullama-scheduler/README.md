...
```sh
# Build
cargo build -p rullama-scheduler --release

# Register with Claude Code (stdio transport)
claude mcp add --transport stdio rullama-scheduler \
  ./target/release/rullama-scheduler

# Or run directly for development
cargo run -p rullama-scheduler
```

## Usage

```sh
rullama-scheduler [OPTIONS]

Options:
  --jobs-dir <PATH>        Storage directory [default: ~/.rullama/scheduler/]
...
```sh
rullama-scheduler --http 127.0.0.1:3200
```

Register the HTTP endpoint separately:

```sh
claude mcp add --transport http rullama-scheduler-http http://127.0.0.1:3200/mcp
```
...
```json
{
  "mcpServers": {
    "rullama-scheduler": {
      "command": "rullama-scheduler",
      "args": ["--jobs-dir", "/var/rullama/scheduler"]
    }
  }
}
```
...
```sh
RUST_LOG=debug rullama-scheduler
RUST_LOG=rullama_scheduler=trace rullama-scheduler
```
