...
//!
//! ```sh
//! claude mcp add --transport stdio rullama-scheduler rullama-scheduler
//! ```
...
#[derive(Parser)]
#[command(
    name = "rullama-scheduler",
    version,
    about = "Local MCP server for cron job scheduling with optional Docker sandboxing"
)]
struct Cli {
    /// Directory for jobs.json and per-job logs (default: ~/.rullama/scheduler/)
    #[arg(long)]
