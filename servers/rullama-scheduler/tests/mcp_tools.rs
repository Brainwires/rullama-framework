//! Integration tests for the MCP tool surface of `rullama-scheduler`.
//!
...
//! would. Mirrors the pattern in `rullama-issues/tests/mcp_tools.rs` and
//! `rullama-brain-server/tests/mcp_tools.rs`.
//!
...
async fn run_job_executes_command() -> Result<()> {
    ...
    const NEEDLE: &str = "rullama-scheduler-test";
